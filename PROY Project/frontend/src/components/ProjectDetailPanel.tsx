...
export default function ProjectDetailPanel({ projectId, onUpdated }: Props) {
  ...
  async function handleProgressChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const updated = await updateProject(project.id, { progress }).catch(console.error)
  ...
  async function handleStatusChange(status: Project['status']) {
    ...
    const updated = await updateProject(project.id, { status }).catch(console.error)
    if (updated) onUpdated?.(updated)
  }
